...
main()
{
  ...
  Shader shader = Shader_construct("./vertexShader.vert", "./fragmentShader.frag");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    Shader_useProgram(shader);

    double timeValue = glfwGetTime();
    float value = (sin(timeValue) / 2.0f) + 0.5f;
    Shader_set3Float(shader, "colorChanger", value, value, value);
